...
export const useIsIE = () => {
	const [_isIE, setIsIE] = useState(false)
...
export const useIsTablet = () => {
	...
	useEffect(() => {
		setTimeout(() => {
			if (_window) {
				setIsTablet(_window.innerWidth <= breakPoint && _window.innerWidth > 500)
			}
		}, 1)
	}, [_window])
...
export const useDebouncedHandleScroll = (callback, interval = 100) => {
...
export const useScrollDirection = ({
	...
	//const scrollPos = (typeof window !== 'undefined' && window.scrollerInstance) ? window.scrollerInstance.scroll.
	const _window = typeof window !== 'undefined' ? window : null

	useEffect(() => {
		setTimeout(() => {
			if (_window) {
				const scrollerInstance = _window.scrollerInstance
				...
				let lastScrollY = scrollerInstance.scroll.instance.scroll.y
				let ticking = false

				const updateScrollDir = () => {
					const scrollY = scrollerInstance.scroll.instance.scroll.y
				...
				if (off) {
					...
					scrollerInstance.on('scroll', onScroll)
				}

				return () => window.removeEventListener("scroll", onScroll)
			}
		}, 1000)
	}, [initialDirection, thresholdPixels, off, _window])
